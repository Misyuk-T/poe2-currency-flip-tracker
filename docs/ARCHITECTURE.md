...
Design notes for PoE2 Currency Flip Tracker: how the radar is computed, what the
gold model assumes, how data is stored and polled, and which assumptions remain
unverified. Moved here from the README, which now covers only what the project is
and how to run it.
